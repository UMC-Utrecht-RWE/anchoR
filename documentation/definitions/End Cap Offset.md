> Optional metadata column `end_cap_offset`, used only by [IN_PRIOR_PREG](IN_PRIOR_PREG.md): caps a prior-episode window's end at `episode_start + end_cap_offset`, if that's earlier than the otherwise-computed `episode_end + end_offset`.
