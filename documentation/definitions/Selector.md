...
| `RANGE_COUNT`    | Counts matches like `COUNT`, then looks up that count in a `concept_ranges` table (matched on `concept_id`, count within inclusive `lower_range`/`upper_range` bounds) to return a bucketed category as `value` instead of the raw count. |
