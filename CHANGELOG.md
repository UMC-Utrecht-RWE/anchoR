...
- `episode_windows()`: the new border-offset formula for turning a selected episode into one or more windows. Six new metadata columns replace everything the removed columns used to do: `anchor_start_offset`/`anchor_end_offset` (see `clip_to_anchor_bounds()` below) and two offset pairs, `before_start_episode_offset`/`after_start_episode_offset` and `before_end_episode_offset`/`after_end_episode_offset`, each relative to the selected episode's own start/end. `before_start_episode_offset` and `after_end_episode_offset` are the "outer" sides (pointing away from the episode); `after_start_episode_offset` and `before_end_episode_offset` are the "inner" sides (pointing into it). Each pair's own contribution is decided independently: a pair with both sides set ("full") is always its own self-contained region, `[edge + before_offset, edge + after_offset]` (an inverted pair, `before_*_offset` later than `after_*_offset`, is an error), and it never suppresses whatever the other pair independently contributes. A non-full pair whose only set side is its outer one pulls the whole computation into shared-window mode, where every non-full pair contributes one point (its one set side, or the unshifted edge if empty) to a single shared window; a full pair sits out of the shared window, keeping its own region alongside it instead. Outside shared-window mode, a non-full pair with only its inner side set forms its own region instead, missing side defaulting to `0`; an empty pair contributes nothing. Neither pair set at all falls back to the episode's own unshifted span. See `documentation/definitions/Episode-Based Window Engine.md` for the complete rule and worked examples.
...
- `validate_variable_ids` to control the values of the `variable_id`, crucial for path formation.
...
- Parquet paths are correctly handled with `DBI::dbQuoteString()` but `DuckDB` attachment paths, export paths, and the temporary directory are interpolated directly. Paths containing an apostrophe will fail and untrusted paths could alter SQL. Added `sql_string` and rerouter the creation of paths
- Made Parquet partition replacement failure-safe: new files are fully staged on the target filesystem before installation, existing partitions are retained as temporary backups, and failed installation attempts roll back to the previous partition instead of leaving missing or partial output.
- All `log_info` have been converted in `log_debug`. It makes the pacakge less noise at the level of the project run.
