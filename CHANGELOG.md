...
- `tests/testthat/helper-fixtures-preg.R`'s `pregnancy_metadata_simple()`/`pregnancy_metadata_complex()` illustrative fixtures re-verified against the `anchor_start_offset`/`anchor_end_offset` clip above; their comments now note where a fixture's own `anchor_start_offset = anchor_end_offset = 0` (previously inert, since only `outside_all_pregnancy` read them) now clips a window down to nothing. Left as-is to illustrate the failure mode rather than changed, since these fixtures aren't asserted against in any test.
