...
testthat::test_that("add_parquet_export builds partitioned parquet SQL", {
  con <- DBI::dbConnect(duckdb::duckdb(), dbdir = ":memory:")
  on.exit(DBI::dbDisconnect(con, shutdown = TRUE), add = TRUE)

  hive_path <- file.path(tempdir(), "researcher's anchor hive")

  sql <- add_parquet_export(
    con = con,
    sql_query = "SELECT * FROM latest_rows",
  ...
  testthat::expect_match(
    sql,
    "FILENAME_PATTERN 'latest_\\{i\\}'",
    perl = TRUE
  )

  # DuckDB escapes an apostrophe in a string literal by doubling it.
  testthat::expect_match(
    sql,
    "researcher''s anchor hive",
    fixed = TRUE
  )
})
...
testthat::test_that("run_selector_queries reports selector context on error", {
  ...
  environment(query_fn) <- list2env(
    list(
      run_selector_query = function(con,
                                    selector,
