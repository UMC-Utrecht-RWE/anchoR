...
test_that("filter_supported_metadata keeps supported rows when unsupported rows are dropped", {
...
test_that("filter_supported_metadata keeps all rows when selectors are supported", {
  metadata <- data.table::data.table(
    variable_id = c("a", "b"),
    concept_id = c("A", "B"),
