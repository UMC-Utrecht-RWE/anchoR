...
| ------ | ------ | ------------------ | ----------- |
| `person_id` | chr | yes | Person identifier used to join concept events to the population input. |
| `concept_id` | chr | yes | Concept identifier matched against `metadata$concept_id`. |
| `date` | Date | yes | Event date used to determine whether a concept record falls inside the requested window. |
| `value` | chr | yes | Value associated with the concept event. In the current concepts output this is often `TRUE` for presence/absence concepts. |
...
| `#ID-000002127#` | `VP_VZV` | `2022-10-04` | `TRUE` |
