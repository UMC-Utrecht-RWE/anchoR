...
The current population object shown in `trial_run.R` contains the following
core columns.
...
Additional date columns may also be used if metadata includes
`anchor_start_col` and/or `anchor_end_col`.

The package does **not** currently expect a long population table with columns
like `anchor_type` and `anchor_date`. The current workflow uses a wide table
with one or more anchor-related date columns.
...
- Extra population columns are allowed and may be useful during window
  construction.
- The current `get_anchor_result()` output does not automatically carry through
  columns such as `match_id`, `boot_id`, or `group`.
- If the same `person_id` appears in multiple population rows with the same
  `T0`, that distinction is not preserved in the current public result shape.
