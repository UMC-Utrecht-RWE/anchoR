...
- Want a wide table (one row per person, one column per question) instead of one row per match? See `vignette("standard-windows", package = "anchoR")` and `vignette("multiple-windows", package = "anchoR")` for `get_anchor_result(..., result_shape = "wide")`.
- Have a question that isn't tied to one fixed date e.g. anything about pregnancy, or another event that can happen more than once? See `vignette("episode-windows", package = "anchoR")`.
